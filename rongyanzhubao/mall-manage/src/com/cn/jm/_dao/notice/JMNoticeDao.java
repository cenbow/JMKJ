...
public class JMNoticeDao extends NoticeDao{
	...
	public Page<Notice> page(int pageNumber, int pageSize,int accountId,int state){
		...
		switch (state) {
		case -1://全部
			format = " FROM tb_notice AS a LEFT JOIN tb_notice_account AS b ON a.id = b.noticeId WHERE (a.type = 1 AND b.accountId = %s) OR a.type = 0 ORDER BY a.id DESC";
			form = JMToolSql.format(format, accountId);
			page =  page(pageNumber, pageSize, " a.type,a.title,a.content,a.image,a.createTime,b.state,b.noticeId,b.readTime", form, true);
			break;
			
		case 0://未读
			format = " FROM tb_notice AS a LEFT JOIN tb_notice_account AS b ON a.id = b.noticeId WHERE (a.type = 1 AND b.accountId = %s AND b.state = 0) OR (a.type = 0 AND b.state IS NULL) ORDER BY a.id DESC";
			form = JMToolSql.format(format, accountId);
			page =  page(pageNumber, pageSize, " a.type,a.title,a.content,a.image,a.createTime,b.state,b.noticeId,b.readTime", form, true);
			break;
			
		case 1://已读
			format = " FROM tb_notice AS a LEFT JOIN tb_notice_account AS b ON a.id = b.noticeId WHERE a.type = 1 AND b.accountId = %s AND b.state = 1 ORDER BY a.id DESC";
			form = JMToolSql.format(format, accountId);
			page =  page(pageNumber, pageSize, " a.type,a.title,a.content,a.image,a.createTime,b.state,b.noticeId,b.readTime", form, true);
			break;
