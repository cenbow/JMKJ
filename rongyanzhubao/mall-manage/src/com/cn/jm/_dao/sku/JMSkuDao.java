...
//					Db.update("delete a,b from shop_sku a right join shop_sku_attr b on a.id = b.skuId where  a.goodsId = ?",goodsId);
...
//						sku.setStock(stock);
	...
	public void updateSkuCode(int attrId,String oldAttr,String attr){
		List<Sku> skuList = Sku.dao.find("select * from shop_sku where id in (select skuId from shop_sku_attr where attrId = ? group by skuId)",attrId);
		for(Sku sku : skuList){
	...
	public List<Sku> selectByIds(String skuIds) {
		...
		StringBuilder preSql = new StringBuilder(
				"select s.*,g.name,g.remark,g.templateId,t.type as templateType");
		return JMCommonDao.jmd.selectList(Sku.class, query,preSql.toString(),"s inner join shop_goods g on s.goodsId = g.id "
