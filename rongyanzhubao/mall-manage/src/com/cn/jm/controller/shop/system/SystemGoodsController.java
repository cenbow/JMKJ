...
public class SystemGoodsController extends JMBaseSystemController {
	...
	public void page() {
		...
		Page<Goods> page = goodsDao.page(pageNumber, pageSize, keyword, artNo, startTime, 
				endTime, state, labelId, account.getId(), false);
		setAttr("state", state);
	...
	public void add() {
		...
		// freightService.selectTemplateList(null));//运费列表
	...
	public void save() {
		...
		for (int i = 0; i < skuNum; i++) {
			...
			// String price = getPara("price"+i);
			// int stock = getParaToInt("stock"+i, 0);
			// String oldprice = getPara("oldprice"+i);
			String attrIds = JMToolString.reSplit(attr, ",");
			String name = getPara("name" + i);
			HashMap<String, Object> skuMap = new HashMap<>();
			skuMap.put("attrids", attrIds);
			// skuMap.put("stock",stock);
			// skuMap.put("oldprice", oldprice);
			// skuMap.put("price", price);
			skuMap.put("name", name);
			// skuMap.put("img",skuimageList.isEmpty() ? "" : skuimageList.get(i));
			skuList.add(skuMap);
		}
		...
		if (goodsDao.save(goods)) {
			...
			// labelRelationDao.addServiceList(servicesIds,goods.getId());//添加服务关系
	...
	public void update() {
		...
		Goods goods = getModel(Goods.class);
		// boolean isEditSku = getParaToBoolean("isEditSku");
		// if(isEditSku){//编辑sku
		for (int i = 0; i < skuNum; i++) {
			int skuId = getParaToInt("skuId" + i);
			String name = getPara("name" + i);
			// String price = getPara("price"+i);
			// int stock = getParaToInt("stock"+i);
			// String oldprice = getPara("oldprice"+i);
			...
			// if(JMToolString.isNotEmpty(img)){
			// sku.setImage(img);
			// }
	...
	public void listSpec() {
		...
		if (shopModId == null) {
			JMResult.success(this, null, "");
		}
		List<Spec> list = specDao.listByShopModId(shopModId);
