...
/**
 * 1.0 : 创建可以应付功能的上传文件工具类
 * 1.1 : 优化一部分上传到本地文件的代码(将视频上传和图片上传统一成一个方法,文件名称更换为  [传入的 path + 雪花生成的数字  + 文件类型] 防止访问量大时造成的文件出现重复问题) 减少1.0的130行代码
...
//	public static final int IMAGE_MAX_SIZE = 200 * 1024 * 1024;
	/** 上传视频允许的最大尺寸，目前只允许 200M */
	public static final int VIDEO_MAX_SIZE = 200 * 1024 * 1024;
	/** 上传的文件路径实际储存位置  */
	// 测试服路径
//	public static final String UPLOAD_FILE_PATH = "/usr/local/apache-tomcat-8.5.11/webapps/buyCarImage/";
	//本地路径
	public static final String UPLOAD_FILE_PATH = "D:/img/shopWebcast/";

	/** 上传的文件访问的路径前缀 */
	public static final String FILE_URI = "http://10.0.0.250/upload/shop-webcast/";
...
//	private static final DefaultCredentialProvider DEFAULT_CREDENTIAL_PROVIDER = new DefaultCredentialProvider(PropKit.get("oss_accessKeyId"), PropKit.get("oss_accessKeySecret"));
//	public static final String ACCESS_KEY_ID;
//	public static final String ACCESS_KEY_SECRET;
	
//	static {
//		PropKit.use("thirdparty.properties");
...
//	}
	 ...
	 * @param upload 需要转换传入的文件
	 * @param path 路径需求前缀加上 /  例如:/image
	...
	private static String uploadFile(File uploadFile, String path) {
		...
		File folder = new File(UPLOAD_FILE_PATH + path + "/" + nowDate);
		...
		File file = new File(folder.toString() + path + SnowflakeUtil.UTIL.nextId() + fileType);
		...
		return file.getPath().replace("\\","/").replace(UPLOAD_FILE_PATH, FILE_URI);
	}
	
	/**
	 * 上传图片
	 * @param controller
	 * @param parameterName 图片的参数名称
	 * @param path 图片指定路径 ,传入值例如 /image 注意加入斜杆
	 * @return
	 */
...
//		list.add(FfmpegUtil.printScreen(UPLOAD_VIDEO_PATH + nowDate + path + "/" + to.getName()));
//		uploadFile.delete();
//		return list;
//	}
	 ...
	 * 上传多张图片
	 * @param controller
	 * @param parameterName  图片的参数名称,注意名称不可以重复
	 * @param path  图片指定路径 ,传入值例如 /image/
	 * @return
	 */
	public static List<String> uploadImgs(Controller controller, String path) {
		...
		File file = new File(UPLOAD_FILE_PATH + UPLOAD_IMG_PATH + nowDate + path);
		...
		for (UploadFile upload : files) {
			...
			if (upload != null) {
				...
				if (f.exists()) {
					...
					f.delete();
					imageList.add(FILE_URI + UPLOAD_IMG_PATH + nowDate + path + "/" + to.getName());
				}
			}
		}
	...
	public static ArrayList<String> uploadImgsByNames(Controller controller, String path, String[] names) {
		...
		if(names != null && names.length != 0) {
			for (String name : names) {
				...
				if(uploadFile != null) {
					String image = uploadFile(uploadFile.getFile(), path);
	 ...
	 * 
	 * @param controller
	 * @param parameterName 传入图片的参数字段
	 * @param path 视频指定路径 ,传入值例如 /video
	 * @return
	 */
	@Deprecated
	...
	public static String uploadImageFile(File f, String path) {
		...
		String nowDate = TimeUtil.formatStr(new Date());
		File file = new File(UPLOAD_FILE_PATH + path + nowDate + "/");
		if (!file.exists()) {
			file.mkdirs();
		}
		...
		return FILE_URI + path + nowDate + "/" + to.getName();
	}
	...
	public static String uploadUrl(String url, String path) {
		...
		String nowDate = TimeUtil.formatStr(new Date());
		File to = new File(UPLOAD_FILE_PATH + UPLOAD_URL_FILE_URI + nowDate + "/" + path + "/file_"
		...
		return FILE_URI + UPLOAD_URL_FILE_URI + nowDate + "/" + path + "/" + to.getName();
	}
...
//		StringBuilder fileName = new StringBuilder().append(System.currentTimeMillis()).append("_").append(RANDOM_NUMBER_GENERATOR.nextInt(999999))
