...
//	 * @return
//	 */
//	public JMResult login(String account, String password, String ip,String userAgent, String ids, String code, String loginType) {
//		//根据type获取登录方法
...
//		}
//		//执行登录方法
//		JMResult result = loginMethod.login(new LoginParam(ids, account, password, code));
//		if(result == null) {
...
//	}
	 ...
	 * @return
	 */
	public JMResult updatePwdByMobile(String mobile, String password) {
		if (StrKit.isBlank(password)) {
			return JMResultUtil.fail(PromptInformationEnum.PASSWORD_NOT_NULL);
		}
		Account account = selectOneByKeyAndValue("mobile =",mobile);
		if (account == null) {
			return JMResultUtil.fail(PromptInformationEnum.USER_NOT_EXISTS);
		}
		return updatePassword(account, password) ? JMResultUtil.success(PromptInformationEnum.SUCCESS) :JMResultUtil.fail(PromptInformationEnum.FAIL);
	...
	public Account getBySessionId(String sessionId) {
		...
		Account account = selectById(session.getAccountId());
		removeSensitiveInfo(account);
		account.put("sessionId", sessionId);
		return account;
	}
	...
	public boolean valiPassWord(Account account, String password) {
		...
		return pwd.trim().equals(mPwd.trim());
	}
	/**
	 * 密码变更
	 * 
	 ...
	 * 商家后台登录
	 */
	public JMResult sysShopLogin(String mobile, String password, Controller controller) {
		password = password.trim();
		Account account = getByMobile(mobile);
...
//	public JMResult bdMail(String mail,int code, String ip,String userAgent,String ids,String bdType){
//		if(isMailExistence(mail) || isExistence(bdType+"=", ids)) {
...
//	}
//	
//	public JMResult bdMobile(String mobile,String code, String ip,String userAgent,String ids,String bdType){
//		if(isExistence("mobile =", mobile) || isExistence(bdType+"=", ids)) {
//			return JMResultUtil.fail(PromptInformationEnum.USER_EXISTS);
//		}
//		/**绑定生成的用户信息密码随机*/
//		Account account = new Account();
...
//	}
	...
	public Account getByMobile(String mobile) {
		return selectOneByKeyAndValue("mobile=",mobile);
	}
	 ...
	 * @return
	 */
	public boolean isExistence(String key,String value) {
		return selectOneByKeyAndValue("id", key, value) != null;
	}
	 ...
	 * @return
	 */
	public boolean isMailExistence(String value) {
		return isExistence("mail =", value);
	}
	 ...
	 * @return
	 */
	public boolean isNotExistence(String key,String value) {
		return !isExistence(key, value);
	}
...
//		String ids = loginParam.getIds();
//		Account account = selectOneByKeyAndValue(field,ids);
//		if(account == null) {
...
//	 * @return
//	 */
//	public JMResult saveAccountByMail(String mail, String password,AccountEnum type){
//		synchronized (mail) {
//			if(isMailExistence(mail)) {
...
//	 * @return
//	 */
//	public JMResult saveAccountByMobile(String mobile, String password,AccountEnum type, String invitationCode){
...
//		String salt = getSalt();
//		password = get256Pwd(salt, password);
//		account.setSalt(salt);
//		account.setPassword(password);
...
//		if(save(account)) {
//			//发送创建关联用户表的信息
//			RedisUtil.sendMessage(Channels.SAVE_ACCOUNT,account.getId().toString());
//			return true;
//		}
//		return false;
//	}
...
//	 * @return 
//	 */
//	public JMResult sendMailCode(String mail,String codeType) {
...
//	 * @return 
//	 */
//	public JMResult sendMobildeCode(String mobile,String codeType) {
//		if (!MobileCodeUtil.checkMobile(mobile)) { //校验发送手机号的是否正确
//			return JMResultUtil.fail(PromptInformationEnum.MOBILE_FORMAT_ERROR);
//		}
//		return sendCode(mobile, codeType, BasicsMethodUtil.MOBILE_CODE_METHOD_CLASS_PATH, (code) -> MobileCodeUtil.sendCode(mobile, code.toString()));
//	}
//	
//	/**
...
//	 * @return
//	 */
//	public JMResult sendCode(String account, String codeType, String classPath, Function<String,Boolean> sendCodeFunction) {
//		//获取是否符合发送验证码方法
...
//			return JMResultUtil.fail(PromptInformationEnum.PARAMETER_ERROR);
//		}
//		JMResult result = codeMethod.check(account);
...
//	
	 ...
	 * @return
	 */
	public boolean checkMobileLoginCode(String mobile, String inputCode) {
		return checkCode(RedisInformation.MOBILE_LOGIN_CODE, mobile, inputCode);
	}
	 ...
	 * @return
	 */
	public boolean checkMobileReplaceCode(String mobile, String inputCode, int accountId) {
		boolean checkFlag = checkCode(RedisInformation.MOBILE_REPLACE_CODE, mobile, inputCode);
		if(checkFlag) {
	 ...
	 * @return
	 */
	public boolean checkMobileRegisterCode(String mobile, String inputCode) {
		return checkCode(RedisInformation.MOBILE_REGISTER_CODE, mobile, inputCode);
	}
	/**
	 * 校验手机号码绑定
	 * 校验用户输入的验证码跟发送的验证码是否一致
	 ...
	 * @return
	 */
	public boolean checkMobileBindingCode(String mobile, String inputCode) {
		return checkCode(RedisInformation.MOBILE_BINDING_CODE, mobile, inputCode);
	}
	/**
	 * 校验手机号码找回密码
	 * 校验用户输入的验证码跟发送的验证码是否一致
	 ...
	 * @return
	 */
	public boolean checkMobileForgetCode(String mobile, String inputCode) {
		return checkCode(RedisInformation.MOBILE_FORGET_CODE, mobile, inputCode);
	}
	 ...
	 * @return
	 */
	public boolean checkMailLoginCode(String mail, String inputCode) {
		return checkCode(RedisInformation.MAIL_LOGIN_CODE, mail, inputCode);
	}
	 ...
	 * @return
	 */
	public boolean checkMailRegisterCode(String mail, String inputCode) {
		return checkCode(RedisInformation.MAIL_REGISTER_CODE, mail, inputCode);
	}
	/**
	 * 校验邮箱绑定
	 * 校验用户输入的验证码跟发送的验证码是否一致
	 ...
	 * @return
	 */
	public boolean checkMailBindingCode(String mail, String inputCode) {
		return checkCode(RedisInformation.MAIL_BINDING_CODE, mail, inputCode);
	}
	/**
	 * 校验邮箱找回密码
	 * 校验用户输入的验证码跟发送的验证码是否一致
	 ...
	 * @return
	 */
	public boolean checkMailForgetCode(String mail, String inputCode) {
		return checkCode(RedisInformation.MAIL_FORGET_CODE, mail, inputCode);
	}
	/**
	 ...
	 * @return
	 */
	private boolean checkCode(String checkType, String mobile, String inputCode) {
		Cache redis = Redis.use();
		String redisKey = checkType + mobile;
		Integer code = redis.get(redisKey);
		//验证码只校验一次,一次之后将键删除
		redis.del(redisKey);
		return code != null && code.toString().equals(inputCode);
	}
	...
	public JMResult replaceMobile(Account account, String mobile, String code) {
		if(isExistence("mobile =", mobile)) {
	...
	public Account selectAccount(String mobile,Integer type) {
		return accountDao.selectAccount(mobile, type);
	}
