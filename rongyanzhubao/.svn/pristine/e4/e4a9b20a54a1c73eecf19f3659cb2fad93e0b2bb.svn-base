...
public class JMGoodsResaleDao extends GoodsResaleDao{
	...
	/** (0新品, 1在售宝贝, 2已售欣赏) */
	static HashMap<Integer,String> sqlMap = new HashMap<>(4);
	static {
		sqlMap.put(1, " AND sg.sellState = 1");
		sqlMap.put(2, " AND sg.sellState = 0");
	}
	/**
	 * @param pageNumber 页数
	 * @param pageSize 页大小
	...
	public Page<GoodsResale> pageResaleGoods(Integer pageNumber, Integer pageSize, Integer accountId, String keyword, Integer type, String state) {
		...
		fromSql.append(" ORDER BY sg.sellState,sg.id DESC");
		return dao.paginate(pageNumber, pageSize, "SELECT sg.id,sg.name,sg.price,sg.resalePrice,sg.thumbnail,sg.video,sgr.id goodsResaleId,IFNULL(sgr.imageState,0) imageState,sgr.describe", fromSql.toString());
	}
