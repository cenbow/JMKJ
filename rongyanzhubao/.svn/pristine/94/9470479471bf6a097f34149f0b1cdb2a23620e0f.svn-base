...
public class JMGoodsDao extends GoodsDao{
	...
	public Page<Goods> pageForApi(String keyword, int labelId,int type, String minprice, String maxprice,
			int serviceType, int pageNumber, int pageSize, Integer userId, Integer isRoomGoods){
		StringBuilder appendSql = new StringBuilder();
		HashMap<String, Object> andpm = new HashMap<>();
		String orderField = "createTime";
		String orderDirection = "DESC";
		andpm.put("state",GoodsEnum.UPPER_SHELF_STATE.getCode());//上架状态的;
		andpm.put("sellState",GoodsEnum.CAN_SELL_STATE.getCode());//未结缘状态的
		//接口多个地方接入,需要根据条件判断是否将直播间归一
		boolean isGourpByRoomId = false;
		...
		if(maxprice != null){
			 appendSql.append(JMToolSql.format("AND price BETWEEN '%s' AND '%s' ",minprice,maxprice));
		}else{
			appendSql.append(JMToolSql.format("AND price >= '%s' ",minprice));
		}
		if(!JMToolString.isEmpty(keyword)){
			keyword = SqlUtil.replaceStr("%"+keyword+"%");
			appendSql.append(" AND (name LIKE ").append(keyword).append(" OR roomName LIKE ").append(keyword).append(" ) ");
			isGourpByRoomId = true;
		...
		if(-1 != isRoomGoods) {
			appendSql.append(" AND roomId IS NOT NULL ");
		}
		//只能搜索出商家已经上架的商品
		andpm.put("accountType", AccountEnum.TYPE_SHOP.getValue());
		if(isGourpByRoomId) {
			appendSql.append(" GROUP BY IFNULL(roomId,-id) ");
		}
		String where = JMToolSql.whereAND(andpm).append(" ").append(appendSql).toString();
		Page<Goods> page = page(pageNumber, pageSize, null, where, orderField, orderDirection, false);
		for(Goods goods : page.getList()){
			...
			goods.put("goodsComment", goodsStarDao.getByGoodsId(goods.getId(), userId));
			goods.put("isCollected", userCollectDao.isCollected(userId, goods.getId()));
			goods.put("isResale", goodsResaleService.isResale(userId, goods.getId()));
			setGoodsIsBroad(goods);
		}
		return page ;
	}
	...
	public Goods detail(int goodsId,Integer userId){
		...
		goods.put("columnList", labelRelationDao.selectRelationList(LabelEnum.SHOP_COLUMN_TYPE, goodsId));
		goods.put("collectedNum", userCollectDao.collectNum(goodsId));// 收藏数
		goods.put("skuList", jmSkuDao.listSKuByGoodsId(goodsId));
		goods.put("specList", jmSkuDao.listByGoodsId(goodsId));
		goods.put("goodsComment", goodsStarDao.getByGoodsId(goodsId, userId));// 商品评论
		goods.put("isCollected", userCollectDao.isCollected(userId, goodsId));// 是否点赞
		goods.put("detailsList", imgDao.selectByTypeAndIds(goodsId, ImgEnum.GOODS_DETAILS_TYPE));// 详情图
		goods.put("ownerList", imgDao.selectByTypeAndIds(goodsId, ImgEnum.GOODS_OWNER_TYPE));// 主图
		goods.put("recommendList", selectRecommendGoods(goods.getLabelId(), userId));// 推荐商品
		goods.put("isResale", goodsResaleService.isResale(userId, goods.getId()));
		setGoodsIsBroad(goods);
		return goods ;
	}
	...
	public JMResult resaleGoodsDetail(int goodsResaleId,Integer userId){
		...
		if(StrKit.notBlank(goodsResale.getDescribe())) {
			goods.setName(goodsResale.getDescribe());
		}
		setGoodsIsBroad(goods);
	...
	private List<Goods> selectRecommendGoods(Integer labelId, Integer accountId) {
		...
		for(Goods goods : goodsList){
			...
			goods.put("isResale", goodsResaleService.isResale(accountId, goods.getId()));
		}
		return goodsList;
	}
	...
	public Page<Goods> pageForApi(Integer columnId, Integer pageNumber, Integer pageSize, Integer userId) {
		...
		fromSql.append(" AND sg.`del` = 0 AND sg.`sellState` = 1");
		fromSql.append(" ORDER BY sg.id DESC");
		Page<Goods> page = dao.paginate(pageNumber, pageSize, "SELECT sg.id id,sg.name,sg.price,sg.thumbnail,sg.video,sg.resalePrice,sg.resaleNum,sg.content,sg.sellState,sg.createTime", fromSql.toString());
		for(Goods goods : page.getList()){
			goods.put("collectedNum", userCollectDao.collectNum(goods.getId()));
			goods.put("goodsComment",goodsStarDao.getByGoodsId(goods.getId(), userId));
			goods.put("isCollected",userCollectDao.isCollected(userId, goods.getId()));
	...
	public Page<Goods> page(Integer pageNumber, Integer pageSize,String keyword,String artNo,String startTime,String endTime,int state,int labelId,boolean isCache){
		...
		StringBuilder fromSql = new StringBuilder(" FROM shop_goods sg ")
			...
			.append(" WHERE sg.state != 2 AND sg.accountType = 2");
	...
	public Page<Goods> selectBroadcastPage(Integer pageNumber, Integer pageSize,String keyword,String artNo,String startTime,String endTime,int state,int labelId,boolean isCache){
		StringBuilder selectSql = new StringBuilder("SELECT sg.id,sg.name,sg.artNo,sg.price,sg.resalePrice,sg.resaleNum,sg.state,sg.createTime,"
				+ "wr.name roomName,tl.name labelName,tau.nick accountNick,ac.mobile as accountMobile");
		StringBuilder fromSql = new StringBuilder(" FROM shop_goods sg ")
			.append(" LEFT JOIN tb_label tl ON sg.labelId = tl.id ")
			.append(" LEFT JOIN webcast_room wr ON wr.id = sg.roomId ")
			.append(" LEFT JOIN tb_account_user tau ON tau.accountId = sg.userId ")
			.append(" LEFT JOIN tb_account ac ON ac.id = tau.accountId")
			.append(" WHERE sg.state != 2");
		if(StrKit.notBlank(keyword)) {
			keyword = SqlUtil.replaceStr("%"+keyword+"%");
			fromSql.append(String.format(" AND (sg.id LIKE %s OR wr.name LIKE %s OR sg.name LIKE %s) ", keyword, keyword, keyword));
		}
		SqlUtil.addWhere(fromSql, " AND sg.type =", GoodsEnum.WEBCAST_GOODS_TYPE.getCode());
		SqlUtil.addWhere(fromSql, " AND sg.state =", state, -1);
		SqlUtil.addWhere(fromSql, " AND sg.artNo =", artNo);
		SqlUtil.addWhere(fromSql, " AND sg.labelId =", labelId, -1);
		SqlUtil.addBetweenTime(fromSql, startTime, endTime, " AND sg.createTime");
	...
	public List<Goods> htmlNewGoods(Integer accountId) {
		...
		sql.append("SELECT sg.id,sgr.id goodsResaleId,sg.thumbnail,DATE_FORMAT(sg.createTime,'%m月%d日') formatTime");
	...
	public int updateGoodsType(int accountId, int type) {
		return Db.update("UPDATE shop_goods SET accountType = " + type + " WHERE userId = " + accountId);
	}
	 ...
	 * @param timeOrder 根据时间倒序还是降序
	 * @param priceOrder 根据金额降序还是倒序
	 * @param isRoomGoods 
	 * @param account 
	 * @return
	 */
	public Page<Goods> pageMyGoods(Integer pageNumber, Integer pageSize, String keyword, String minprice,
			String maxprice, String startTime, String endTime, Integer labelId, String timeOrder, String priceOrder, Integer sellState, Account account) {
		StringBuilder selectSql = new StringBuilder("SELECT sg.id,sg.name,sg.artNo,sg.price,sg.thumbnail,sg.video,sg.createTime");
		StringBuilder fromSql = new StringBuilder(" FROM shop_goods sg ")
			.append(" LEFT JOIN tb_label tl ON sg.labelId = tl.id ")
			.append(" WHERE sg.state != 2 AND sg.sellState = ").append(sellState)
			.append(" AND sg.userId = ").append(account.getId());
		if(StrKit.notBlank(keyword)) {
			keyword = SqlUtil.replaceStr("%"+keyword+"%");
			fromSql.append(String.format(" AND (sg.name LIKE %s OR sg.artNo LIKE %s) ", keyword, keyword));
		}
		if(StrKit.notBlank(maxprice)) {
			fromSql.append(String.format(" AND price BETWEEN %s AND %s ", SqlUtil.replaceStr(minprice), SqlUtil.replaceStr(maxprice)));
		}
		SqlUtil.addWhere(fromSql, " AND sg.labelId =", labelId, -1);
		SqlUtil.addBetweenTime(fromSql, startTime, endTime, " AND sg.createTime");
		fromSql.append(" ORDER BY ");
		if(StrKit.notBlank(priceOrder)) {
			fromSql.append("sg.price ").append(priceOrder).append(",");
		}
		fromSql.append(" sg.createTime ").append(timeOrder);
		String strFromSql = SqlUtil.getSql(fromSql);
		String strSelectSql = selectSql.toString();
		return dao.paginate(pageNumber, pageSize, false, strSelectSql,strFromSql);
	}
	 ...
	 * @param goods
	 */
	private void setGoodsIsBroad(Goods goods) {
		Room room = roomService.selectById(goods.getRoomId());
		goods.put("isBroad", room != null?RoomEnum.ONLINE.equals(room.getState().intValue()):false);
		goods.put("room", room);
	}

	public String labelNameById(Integer labelId) {
		Label twoLabel = labelDao.getById(labelId);
		if(twoLabel == null) {
			return "";
		}
		Label oneLabel = labelDao.getById(twoLabel.getParent());
		String oneName = oneLabel != null ? oneLabel.getName() : "";
		return oneName + " " + twoLabel.getName();
	}
	
	/**
	 * 查找关联该直播间的商品
	 */
	public List<Goods> selectGoodsByRoomId(Integer roomId) {
		String sql = "select * from shop_goods where roomId = ?";
		return Goods.dao.find(sql,roomId);
	}
	

	public void setGoodsRoomName(Goods goods) {
