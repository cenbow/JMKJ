...
public class JMAccountUserDao extends AccountUserDao {
	 ...
	 * @return
	 */
	public boolean changeBalance(int accountId, BigDecimal amount) {
		return Db.tx(()->{
			AccountUser accountUser = getByAccountId(accountId);
			int version = accountUser.getVersion();
			return Db.update("UPDATE tb_account_user SET amount = amount + ?,version = version + 1 WHERE accountId = ? AND version = ?", amount, accountId, version) == 1;
		});
	}
	 ...
	 * @return
	 */
	public boolean updateMobileByAccountId(String mobile, Integer accountId) {
		return Db.update("UPDATE tb_account_user SET mobile = ? WHERE accountId = ?", mobile, accountId) == 1;
	}
