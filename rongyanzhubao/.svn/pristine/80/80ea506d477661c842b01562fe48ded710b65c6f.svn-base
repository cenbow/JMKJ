...
//		Integer id = getParaToInt("id",null);
		Integer payType = getParaToInt("payType", -1);
		...
		Page<OrderGoods> page = orderDao.pageSys(keyword, addressName, startTime, endTime, state, pageNumber, pageSize, address, goodsName, payType);
	...
	public void detailData(){
		...
		order.put("buyName",accountUserService.myInformation(order.getAccountId()));
	...
	public void selectRefundDetail(){
		...
		for(RefundOrder refundOrder : refundOrders){
			if(refundOrder != null){
				...
				refundOrder.put("buyUser", accountUserService.selectByAccountId(refundOrder.getUserId()));
	...
	public void handleRefund(){
		...
		if(refundOrder.getType() == 0 ){//仅退款(发货前)
			...
			int dealState;
			//5已通过等待上传物流单号退货7上传物流单号后不通过8上传物流单号后通过等待退款
			if(state == 2) {
				dealState = refundOrder.getState() == 0 ? 2 : 7;
			}else {
				dealState = refundOrder.getState() == 0 ? 5 : 8;
			}
