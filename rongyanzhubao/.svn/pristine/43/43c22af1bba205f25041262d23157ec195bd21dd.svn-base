...
//	}
	 ...
	 *
	 */
	public boolean add(String name,String [] attrs){
		try {
			return Db.tx(new IAtom() {
				@Override
				public boolean run() throws SQLException {
...
//					spec.setShopMdId(shopMdId);
	...
	public List<Spec> list(){
		List<Spec> list = Spec.dao.find("select ss.id,ssa.id attrsId,ssa.name from shop_spec ss INNER JOIN shop_spec_attribute ssa ON ssa.specId = ss.id ");
//		for(Spec spec : list){
//			spec.put("attrs",specAttributeDao.listAttrsBySpecId(spec.getId()));
//		}
		return list ;
	 ...
	 *
	 */
	public List<Spec> newList(){
		List<Spec> list = Spec.dao.find("select ss.id,ssa.id attrsId,ssa.name skuCode from shop_spec ss INNER JOIN shop_spec_attribute ssa ON ssa.specId = ss.id ");
//		for(Spec spec : list){
//			spec.put("attrs",specAttributeDao.listAttrsBySpecId(spec.getId()));
//		}
		return list ;
