...
public class SystemAdminController extends JMBaseSystemController {
	...
	public void page(){
		...
		if (JMToolString.isNotEmpty(startTime) && JMToolString.isNotEmpty(endTime)) {
			page = accountService.page(pageNumber, pageSize, "", andpm, orpm, likepm, startTime,endTime,"id", JMBaseDao.DESC, false);
		}else {
			page = accountService.page(pageNumber, pageSize, "", andpm, orpm, likepm, "id", JMBaseDao.DESC, false);
		}
	...
	public void setState() {
		Integer id = getParaToInt("id");
		accountService.replaceState(id);
