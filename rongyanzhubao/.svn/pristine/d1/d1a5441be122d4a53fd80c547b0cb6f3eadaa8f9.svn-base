...
public class JMImgDao extends ImgDao{
	...
	public Page<Img> page(Integer pageNumber, Integer pageSize,Integer id,String keyword,String startTime,String endTime,boolean isCache){
	 ...
	 * @return
	 */
	public int inserGoodsImages(Integer goodsId, ArrayList<String> detailImageList, ArrayList<String> goodsImageList, String formatDate) throws Exception {
		...
		for (String detailImage : detailImageList) {
			insertSql.append(leftBrackets)
			.append(symbol).append(detailImage).append(symbol).append(comma)
			.append(symbol).append(ImgEnum.GOODS_DETAILS_TYPE).append(symbol).append(comma)
			.append(symbol).append(goodsId).append(symbol).append(comma)
			.append(symbol).append(formatDate).append(symbol).append(rightBrackets).append(comma);
		}
		for (String goodsImage : goodsImageList) {
			insertSql.append(leftBrackets)
			.append(symbol).append(goodsImage).append(symbol).append(comma)
			.append(symbol).append(ImgEnum.GOODS_OWNER_TYPE).append(symbol).append(comma)
			.append(symbol).append(goodsId).append(symbol).append(comma)
			.append(symbol).append(formatDate).append(symbol).append(rightBrackets).append(comma);
		}
	 ...
	 * @return
	 */
	public int inserGoodsImages(Integer ids, ArrayList<String> imgList, String formatDate, ImgEnum imageType) throws Exception {
		final char leftBrackets = '(';
		final char rightBrackets = ')';
		final char symbol = '\'';
		final char comma = ',';
		...
		for (String image : imgList) {
			insertSql.append(leftBrackets)
			.append(symbol).append(image).append(symbol).append(comma)
			.append(symbol).append(imageType).append(symbol).append(comma)
			.append(symbol).append(ids).append(symbol).append(comma)
			.append(symbol).append(formatDate).append(symbol).append(rightBrackets).append(comma);
		}
		return Db.update(insertSql.substring(0, insertSql.length()-1));
	}
	...
	/**
	 * @param ids 关联的表id
	 * @param imgType 图片关联类型
	 * @param notInIds 要删除不包含在id里的数据
	 * @return 
	 */
