 ...
 */
@JMRouterMapping(url = SystemGoodsModelsController.url)
@Before(value={SystemLoginInterceptor.class})
public class SystemGoodsModelsController extends JMBaseSystemController {
	
	public static final String path = JMConsts.base_view_url+"/system/shop/goodsmodels";
	public static final String url ="/system/goods/models";
	...
	public void page(){
		...
		Page<GoodsModels> page = null;
		if (JMToolString.isNotEmpty(startTime) && JMToolString.isNotEmpty(endTime)) {
			page = goodsmodelsDao.page(pageNumber, pageSize, "", andpm, orpm, likepm, startTime,endTime,"id", JMBaseDao.DESC, true);
		}else {
			page = goodsmodelsDao.page(pageNumber, pageSize, "", andpm, orpm, likepm, "id", JMBaseDao.DESC, true);
		}
	...
	public void add(){
		...
		setAttr("goodsModels", new GoodsModels());
		setAttr("spec", new Spec());
		render(path+"/add.html");
	}

	public void save(){
		GoodsModels goodsmodels = getModel(GoodsModels.class);
		goodsmodels.setCreateTime(new Date());
		boolean result = goodsmodelsDao.save(goodsmodels);
		if(result){
	...
	public void edit(){
		...
		setAttr("goodsModels", goodsmodelsDao.getById(id));
		render(path+"/edit.html");
	}

	public void update(){
		GoodsModels goodsmodels = getModel(GoodsModels.class);
		boolean result = goodsmodelsDao.update(goodsmodels);
		if(result){
	...
	public void delete(){
		...
		if (goodsmodelsDao.deleteById(id)) {
	...
	public void dels(){
		...
		if(ids != null){
			boolean result = goodsmodelsDao.deleteByIds(ids);
			if (result) {
				JMResult.success(this, "删除成功");
			}else {
				JMResult.fail(this, "删除失败");
