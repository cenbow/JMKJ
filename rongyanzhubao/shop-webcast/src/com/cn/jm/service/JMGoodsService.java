...
public class JMGoodsService extends BasicsService<Goods>{
	...
	public JMResult apiSaveGoods(Controller controller, String video, int skuNum, Goods goods, Account account, ArrayList<String> detailImageList) {
		List<HashMap<String, Object>> skuList = new ArrayList<>();
		for(int i = 0;i<skuNum;i++){
			Integer[] attr = controller.getParaValuesToInt("attrids"+i);
		...
		if(Db.tx(() ->{
			try {
				if(save(goods)) {
					JMCacheKit.removeAll(JMGoodsDao.cacheName);
			...
			return JMResultUtil.success(goods.getId());
		return JMResultUtil.fail();
	}

	public JMResult apiUpdateGoods(Controller controller, String video, Integer nowSkuNum, Integer oldSkuNum,
	 ...
	 * @return
	 */
	public List<Goods> selectMallGoods() {
		return selectByKeysAndValues("id,name", new String[] {"AND accountType =", "AND type ="}, new Object[] {AccountEnum.TYPE_SHOP, GoodsEnum.SHOP_GOODS_TYPE});
	}

	/**
	 * 根据商品获取对应的参数列表和新的参数列表
	 * @param goodsId
	 * @return 
	 */
	public List<Sku> listSkuDetail(Integer goodsId) {
		return skuDao.listSkuDetail(goodsId);
	}
