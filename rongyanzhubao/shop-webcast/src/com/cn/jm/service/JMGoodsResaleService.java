...
public class JMGoodsResaleService extends BasicsService<GoodsResale> {
	...
	public JMResult resaleGoods(Integer accountId, Integer goodsId, String describe) {
		...
		if(goods.getUserId().equals(accountId)) {
			return JMResultUtil.fail(PromptInformationEnum.NOT_RESALE_ME_GOODS);
		}
		...
		if(goodsResale == null) {
			...
			return JMResultUtil.success(goodsResale.getId());
		}
		return JMResultUtil.fail(PromptInformationEnum.ALREADY_RESALE_GOODS);
	}
	...
	public JMResult updateMyResaleGoods(int accountId, ArrayList<String> detailImageList, int goodsId, String describe, String[] notInIds) {
		...
		try {
			...
			int insertNum = detailImageList == null ? 0 : detailImageList.size();
			int delteNum = imgService.deleteResaleGoodsImage(id, notInIds);
			imgService.inserImages(id, detailImageList, ImgEnum.RESALE_GOODS__DETAILS_TYPE);
	 ...
	 * @return
	 */
	public boolean isResale(Integer accountId, Integer goodsId) {
		if(accountId == null || goodsId == null) {
			return false;
		}
		return goodsResaleDao.getByAccountIdAndGoodsId(accountId, goodsId) != null;
	}
	 ...
	 * @return
	 */
	public JMResult getGoodsResaleUrl(Integer goodsResaleId) {
		GoodsResale goodsResale = selectById(goodsResaleId);
		if(goodsResale == null) {
			return JMResultUtil.fail(PromptInformationEnum.GOODS_NOT_EXISTENCE);
		}
		return JMResultUtil.success(String.format(BasicsInformation.HTML_GOODS_RESALE_URL, goodsResale.getGoodsId(), goodsResaleId));
	}
