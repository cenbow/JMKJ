...
public class RedisUtil{
	...
	private static final String COOKIE_TO_KENKEY = "user_toKen";
	 ...
	 * 设置一个数据定时器,之后执行一个方法
	 */
	public static void putTimingData(int overdueTime, String key, String methodName) {
		Jedis jedis = Redis.use().getJedis();
		String newKey = RedisInformation.getTimingKey(methodName, key);
		try {
			jedis.select(RedisInformation.TIMER_DATABASE_NUM);
			jedis.setex(newKey, overdueTime, RedisInformation.NULL);
		} finally {
			jedis.close();
		}
	}
