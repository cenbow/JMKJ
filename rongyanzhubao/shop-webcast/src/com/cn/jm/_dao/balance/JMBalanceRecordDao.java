...
//	public JMBalanceDao balanceDao ;
	 ...
	 * @param ids 				来源用户id
	 * @param sourceId 			订单id或者提现申请id
	 * @param type 				0支出 1收入
	 * @param msgType 			1邀请返利2提现3提现失败
	 * @param message 			描述
	 * @param amount 			变更多少余额
	 * @return
	 */
	public boolean saveRecord(int accountId, int ids, int sourceId, ExtractEnum type, ExtractEnum msgType, String message, BigDecimal amount) {
		BalanceRecord accountRecord = new BalanceRecord();
		accountRecord.setAccountId(accountId);;
		accountRecord.setType(type.getValue());
		accountRecord.setMsgType(msgType.getValue());
...
//		AccountBalance coinBalance = balanceDao.getByUserId(accountId);
		AccountUser accountUser = jmAccountUserDao.getByAccountId(accountId);
		accountRecord.setBalance(accountUser.getAmount());
	 ...
	 * @return
	 */
	public Page<BalanceRecord> page(int pageNumber, int pageSize, Integer accountId, Integer type, Integer msgType, String startTime, String endTime, boolean isCache){
		StringBuilder selectSql = new StringBuilder("SELECT br.id,br.type,br.msgType,br.message,br.amount,br.balance,br.createTime,el.state");
		StringBuilder fromSql = new StringBuilder(" FROM ");
		fromSql.append(tableName + " br");
		fromSql.append(" LEFT JOIN tb_extract_log el ON br.sourceId = el.id");
		fromSql.append(" :WHERE ");
		SqlUtil.addWhere(fromSql," AND br.accountId = ", accountId);
		SqlUtil.addWhere(fromSql," AND br.type = ", type, -1);
		SqlUtil.addWhere(fromSql," AND br.msgType = ", msgType);
		SqlUtil.addBetweenTime(fromSql, startTime, endTime, " AND br.createTime");
		fromSql.append(" ORDER BY br.id DESC");
	...
	**
	 */
//	public void saveCoin(int accountId, int type, int msgType2,int coinTypeId,String message, 
